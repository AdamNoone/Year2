...
INSERT INTO Product ( stockCode, c,costPrice, retailPrice,  supplierID) 
...
-- 10 marks 
  ...
  select stockDescription --the name of the item 
  from Product--from teh product table
  left outer join prescriptionItem using (stockCode) --left outer join shows everything from Product with a matchimg record in prescriptionItem
  Where prescriptionID is null; --if id is null then product hasnt been used  
...
-- 15 marks
  ...
  Select StockDescription ,REPLACE(count (prescriptionID),0,'None') --the name of the item  & count num of times product prescribe(if its 0 displays null)
  From Product --from product table
  left outer Join Prescriptionitem  --left outer join shows everything from Product with a matchimg record in prescriptionItem
  ON Product.stockCode = PrescriptionItem.stockCode -- joins tables using stock code
  GROUP BY stockDescription;  -- groups stockDescription (name) of each items so they are onlt displayed in 1 row 
...
Select StockDescription ,REPLACE(count (prescriptionID),0,'None')--the name of the item  & count num of times product prescribe(if its 0 displays null)
  ...
  ON Product.stockCode = PrescriptionItem.stockCode -- joins tables using stock code
  GROUP BY stockDescription -- groups stockDescription (name) of each items so they are onlt displayed in 1 row 
  Having count (prescriptionID) < 1;-- only display records that that have a count of less than 1 (ie 0 )
...
-- 10 marks
  ...
  select custName as "Customer Name",nvl(prescriptionID,0) as prescriptionID -- gets customersname & and prescriptionID (if null change to 0)
  from Customer --from Customer table
  left outer join prescription using (CustID);--left outer join shows everything from Customer with a matching record in prescription 
												--join using customerID
...
--gets the customers name & Docs name (outputs NA if no prescription ) & prescriptionID (outputs NA if no prescription )
select CustName as "Customer Name" ,nvl(DocName,'NA') as "Doctor's Name", nvl( to_char(prescriptionID), 'NA' ) as "Prescription ID"
from prescription--from Prescription table
inner join Doctor using (DocID) --returns matching rows in prescription and Doctor table  (joint using DocID)
right outer join Customer on Customer.CustID= prescription.CustID;--Return the set of records from Customer, with the matching records (where available) in prescription.
...
CREATE VIEW  CustomerPrescription  AS --creates a view called CustomerPrescription
				select CustName as "Customer Name" ,DocName as "Doctor's Name",prescriptionID as "Prescription ID",custAddress, custPhone -- gets customers name ,doctors name,prescriptionID,answel as the customers address and phone num
				from Customer -- rom the customer table 
				right outer join prescription on Customer.CustID= prescription.CustID--Return the set of records from Prescription, with the matching records (where available) in Customer.
				inner join Doctor using (DocID);--returns matching rows in Customer and Doctor table  (joint using DocID)
				
				SELECT * FROM CustomerPrescription; --line to prove view was crated successfuly ,we can display the view
...
select  "Customer Name" ,"Doctor's Name","Prescription ID" --  values of  "Customer_Name" ,"Doctor's_Name","Prescription_ID" of are giotten it the derived table below
from  
      (select  CustName as "Customer Name" ,nvl(DocName,'NA') as "Doctor's Name", nvl( to_char(prescriptionID), 'NA' ) as "Prescription ID" from Doctor -- calculates/gets "Customer_Name" ,"Doctor's_Name","Prescription_ID" as calculated below
left outer join prescription using (DocID) --returns matching rows in Doctor and Prescription table  (joint using DocID)
right outer join Customer on Customer.CustID= prescription.CustID----Return the set of records from Customer, with the matching records (where available) in Doctor.
);
	...
	CREATE VIEW  PrescriptionDetail  AS --creates a view called PrescriptionDetail
							Select PrescriptionID, StockDescription,  staffName as DispensedBy --gets PrescriptionID, StockDescription,  staffNam
							From PrescriptionItem--from  PrescriptionItem table
							inner join Product using (stockcode) --returns matching rows in prescriptionItem and Product table  (joint using stockcode)
							inner join Staff using (staffID); --returns matching rows in prescriptionItem and staff table  (joint using staffID)
