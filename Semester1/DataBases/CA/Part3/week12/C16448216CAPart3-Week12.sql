...
--25 marks
		    ...
		    SELECT stockDescription AS "Product Name", staffName AS "Staff Member" -- gets the name of each prodcut and the staff memeber who prescribed it 
			 FROM Prescription -- from the Pescription table 
				INNER JOIN Staff USING (staffID) --returns matching rows from Prescription & Staff
				INNER JOIN PrescriptionItem USING (PrescriptionID)
				INNER JOIN Product USING (StockCode)
						
	   	UNION--results from both queries after eliminating duplications.
			SELECT stockDescription AS "Product Name", staffName AS "Staff Member" -- gets the name of each prodcut and the staff memeber who sold it 
				FROM PrescriptionItem -- from the PescriptionItem table 
				INNER JOIN Staff USING (StaffID)--returns matching rows from PrescriptionItem & Staff
				INNER JOIN Product USING (StockCode);--returns matching rows from PrescriptionItem & Product 
...
SELECT stockDescription AS "Product Name", staffName AS "Staff Member" -- gets the name of each prodcut and the staff memeber who prescribed it 
	   	...
	   	Intersect--results from both queries after eliminating duplications.
			SELECT stockDescription AS "Product Name", staffName AS "Staff Member" -- gets the name of each prodcut and the staff memeber who sold it 
				FROM PrescriptionItem -- from the PescriptionItem table 
				INNER JOIN Staff USING (StaffID)--returns matching rows from PrescriptionItem & Staff
				INNER JOIN Product USING (StockCode);--returns matching rows from PrescriptionItem & Product 
...
-- You need to think about the naming of your columns
	 select custName as "Name/Description" -- gets customers name  
	 from Customer -- from customer table
	 WHERE custName LIKE 'B%' --where the customer name starts with a B
 union --results from both queries after eliminating duplications.
	 select staffName  --get name of staff memeber 
	 from Staff---from the staff table
	 WHERE staffName LIKE 'B%' --where the staff name starts with a B
union --results from both queries after eliminating duplications.
	 select stockDescription --gets name of product 
	 from Product --from the product table 
	 WHERE stockDescription LIKE 'B%'; --where the products name starts with a B


--4. Write the sql using a SET operation to output the names of doctors that have not appeared on a prescription
-- 20 marks

select DocName as "Doctor's Name" --gets the doctors name 
from Doctor --from the doctor table
minus --then take out of the table rows with doctors that appear in a prescripton
select DocName --gets doc name 
from Prescription -- from the prescription table 
inner join Doctor using (docID); --returns all rows (docID's) that match in the prscription and Doctor table
...
select DocName as "Doctor's Name"--gets doctors names
from prescription --from teh prscription table
inner join Doctor using (DocID);--returns all rows (docID's) that match in the prscription and Doctor table.
...
--5 marks
	select docName as "Doctor's Name",count(PrescriptionID) as "Amount of Prescriptions" --gets doctors name & counts prescription ID's
	from DOCTOR --from the table doctor
	join prescription using (DocID) --joins the prsecription table using DocID
	join prescriptionItem using (prescriptionID) --joins the prsecriptionItem table using prescriptionID
	Where docID = prescriptionID -- where docID and prescriptionID are the same 
	group by docName; -- groups Doctors name so each doctor is only diplayed once. 
...
--gets the doctors name ,the nuber of prscriptins each doctor has (output the word none if they have no prescriptions)
SELECT docName as "Doctors Name", DECODE(COUNT(Prescription.prescriptionID), 0, 'NONE', COUNT(Prescription.prescriptionID))AS "Amount of prescriptions"
from Doctor --from the table doctor
left join Prescription using (docID)--left o join shows everything from Doctor with a matchimg record in prescription
GROUP BY docName-- groups Doctors name so each doctor is only diplayed once.
HAVING COUNT(Prescription.prescriptionID) = 0; --will only show doctors that dont have a prescriptionID
...
CREATE VIEW  DoctorLeagueTable  AS --craetes a view called DoctorLeagueTable

select  docName as "DoctorName",count(PrescriptionID) as "NumPrescriptions"--gets doctors name & counts PrescriptionID
	from DOCTOR--from the doctor table 
	join prescription using (DocID) --joins the prsecription table using DocID
	join prescriptionItem using (prescriptionID)--joins the prsecriptionItem table using prescriptionID
	Where docID = prescriptionID -- where docID and prescriptionID are the same
...
UNION --results from both queries after eliminating duplications.
	 SELECT  docName as "Doctors Name",COUNT(prescriptionID)AS "Amount of prescriptions" --gets doctors name & counts PrescriptionID
	from Doctor --from the doctor table 
	left join Prescription using (docID) --left outer join shows everything from Doctor with a matchimg record in prescriptionItem
	GROUP BY docName-- groups Doctors name so each doctor is only diplayed once.
	HAVING COUNT(Prescription.prescriptionID) = 0; --will only show doctors that dont have a prescriptionID
