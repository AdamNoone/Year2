...
void  parse(char *line, char **argv)//a way of extracting certain elements from a string
{
...
void  execute(char **argv)pionter to arguments
{
     pid_t  pid;//process id
     int   status;//used by parent to see if the child has finished
 ...
 //    printf("test execute fnt\n");	
     ...
     if ((pid = fork()) < 0) {     /* fork a child process           */
          ...
          while (wait(&status) != pid);       /* wait for completion (waits for child ie program called to finish) */
...
void  main(void)
{
     ...
     while (1) {                   /* repeat until done ....         */
         ...
         fgets(line,1024,stdin);  /*   user types in what program they wan to run and shel read in the command line     */
      //    printf("\ntest %s\n",line);
          parse(line, argv);       /* parse the line (just a way of extracting certain elemnts from the string entered)            */
          if (strcmp(argv[0], "exit") == 0)  /* is it an "exit"?     */
          { 	printf("exit program");	
               exit(0);            /*   exit if it is                */
	  }	
          execute(argv);           /* otherwise, execute the command passing in file nmae and its arguments */
     }
