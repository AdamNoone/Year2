...
insert into event (eventID, eventName, eventDate, sportCode,prizeFund) values (6, '400m Mens Final', '14 Aug 2016',2,9000);
insert into event (eventID, eventName, eventDate, sportCode,prizeFund)values (7, 'Womens Lightweight', '14 Aug 2016',3,9000);
...
select * from eventenrollment;
     ...
     SELECT competitor.compName, EventResult.position
     FROM EventResult
     JOIN competitor ON competitor.compID=EventResult.compID;
     
     SELECT position
     FROM EventResult;
     
     Select LOWER(position) from EventResult;
